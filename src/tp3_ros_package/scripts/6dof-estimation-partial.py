...
class Estimation_Node:
	...
	if any(len(self.plane_points[color]) <= self.num_of_plane_points for color in ["red", "green", "blue"]):
		...
		H = np.array(self.plane_points[color])  # H is matrix containing all points in the current color plane. H size is (number of points,3) 
		...
		b = np.array([1,1,1]).T # Matrix 1 with size (Dimension of points,1) due to set plane equation ax+by+cz = 1 with d = -1
